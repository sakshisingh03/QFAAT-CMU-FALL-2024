...
def execute_macd(ticker, data, fast_period, slow_period, signal_period):
    ...
    stats = mm.execute_macd_strategy(
            data, mm.MACDStrategy, fast_period=fast_period, slow_period=slow_period, signal_period=signal_period
        )
...
def optimize_macd(ticker, data, fast_periods, slow_periods, signal_periods):
    report = []
    metric = "Sharpe Ratio"
    stats, heatmap = mm.optimize_macd_strategy(data, mm.MACDStrategy, fast_periods, slow_periods, signal_periods, metric)
...
def train_test_split_optimize_macd(ticker, data, fast_periods, slow_periods, signal_periods):
    report = []
    metric = "Sharpe Ratio"
    ...
    train_stats, train_heatmap = mm.optimize_macd_strategy(
        train_data, mm.MACDStrategy, fast_periods, slow_periods, signal_periods, metric
    )
    ...
    heatmap_data = temp_df.pivot_table(index="fast_period", columns="slow_period", values="Sharpe Ratio")
...
if __name__ == "__main__":
    if len(sys.argv) != 3:
        ...
        print("Usage: run_macd.py <purpose>")
        sys.exit(1)
    ...
    if purpose == "execute":
        ...
        for ticker in tickers:
            execute_macd(ticker, data[ticker], fast_period, slow_period, signal_period)
            
        # combine execution results in one csv
        utils.combine_execution_results(instrument_type, tickers, 'outputs/macd/')
    elif purpose == "optimize":
        fast_periods = range(5, 50)       
        slow_periods = range(5, 50)   
        signal_periods = range(5, 50) 
        for ticker in tickers:
            optimize_macd(ticker, data[ticker], fast_periods, slow_periods, signal_periods)
            
        # combine optimization results in one csv
        utils.combine_optimization_results(instrument_type, tickers, 'outputs/macd/')
    elif purpose == "train_test_split":
        fast_periods = range(5, 50)
        slow_periods = range(5, 50)
        signal_periods = range(5, 50)
        for ticker in tickers:
            train_test_split_optimize_macd(ticker, data[ticker], fast_periods, slow_periods, signal_periods)
        
        # combine train test results in one csv
        utils.combine_train_test_results(instrument_type, tickers, 'outputs/macd/')
    else:
